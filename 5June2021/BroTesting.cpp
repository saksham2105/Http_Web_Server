...
friend long getContentLength(Response &);
friend string getContent(Response &);
friend string getContentType(Response &);
};
long getContentLength(Response &response)
{
return response.contentLength;
}
string getContent(Response &response)
{
string content="";
response.contentIterator=response.content.begin();
while(response.contentIterator!=response.content.end())
{
content+=(string)(*response.contentIterator).c_str();    
...
string url;
stringstream ss((string)requestData);
int i=0;
while(ss >> url) 
{
if(i==1)
...
void (*callBack)(Request &,Response &)=NULL;
for(auto i : this->urlMappings)
{
//check if this url exists in map    
if(i.first.compare(url)==0)
{
//extract address of a function    
callBack=i.second;
break;
}
}
...
printf("%s",requestData);
printf("%s\n",getContent(response).c_str());
printf("%ld\n",getContentLength(response));
printf("%s\n",getContentType(response).c_str());
string responseToSend="";
responseToSend+="HTTP/1.1 200 OK\r\n";
responseToSend+="Connection: close \r\n";
responseToSend+="Content-Type: "+getContentType(response)+"\r\n";
responseToSend+="Content-Length: "+to_string(getContentLength(response))+"\r\n\r\n";
responseToSend+=getContent(response);
send(clientSocketDescriptor,responseToSend.c_str(),strlen(responseToSend.c_str()),0);
cout<<responseToSend<<endl;
char b[100];
continue;
}
cout<<"URL Not Mapped"<<endl;
const char *res=
...
send(clientSocketDescriptor,res,strlen(res),0);
delete [] requestData;
